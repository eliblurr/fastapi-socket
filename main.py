...
class ConnectionManager:
    ...
    def client_connection(self, client_id:int):
        return [websocket for websocket in self.active_connections if websocket._cookies.get('client_id', None) == client_id][0]

    async def send_personal_message(self, message:(str, dict), client_id:int):
        websocket = self.client_connection(client_id)
...
async def websocket_endpoint(websocket: WebSocket, client_id:int = Cookie(...)):
    print(dir(websocket))
    ...
    try:
        while True:
            ...
            # f"You wrote: {data}"
            await manager.send_personal_message("['sd','sd']", websocket, client_id)
